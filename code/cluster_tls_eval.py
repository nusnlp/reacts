...
def process_trial(collections, timeline_path, args, embedding_func, overall_results):
    ...
    for keyword, index in tqdm(TARGET_KEYWORDS[args.dataset]):
        ...
        for const, timelines in col.all_timelines.items():
            events, content2type = load_events(os.path.join(args.events_path, under_key, const, f"{under_key}_events.jsonl"))
            for tl_index, gt_timeline in enumerate(timelines):
                ...
                timeline_res = process_timeline(gt_timeline, summary_length, under_key, tl_index, timeline_path, const, args, embedding_func, content2type)
...
def process_timeline(gt_timeline, summary_length, keyword, tl_index, timeline_path, const, args, embedding_func, content2type):
    docs = pickle.load(open(os.path.join(timeline_path, keyword, const, 'docs.pickle'), 'rb'))
    event_pool = pickle.load(open(os.path.join(timeline_path, keyword, const, 'event_pool.pickle'), 'rb'))

...
class ConstrainedDataset:
    ...
    def _load_collections(self):
        ...
        for topic in self.topics:
            topic_path = self.path / topic
            c = ConstrainedArticleCollection(topic_path)
            collections[topic] = c
        return collections


class ConstrainedArticleCollection:
    def __init__(self, path, start=None, end=None):
        self.name = os.path.basename(path)
        self.path = pathlib.Path(path)
        self.all_timelines = self._load_timelines()
        self.start = start
        self.end = end

    def _load_timelines(self):
        all_timelines = {}
        for const in sorted(os.listdir(self.path)):
            if not os.path.isdir(os.path.join(self.path, const)):
                continue
            timelines = []
            # path = self.path / 'timelines.jsonl'
            path = pathlib.Path(os.path.join(self.path, const, 'timelines.jsonl'))
            if not path.exists():
                return []

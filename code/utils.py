...
def count_event(args):
    ...
    for subdir in os.listdir(args.input):
        dir_path = os.path.join(args.input, subdir)
        file_path = os.path.join(dir_path, 'timelines.jsonl')
        with open(file_path) as f:
            datum = json.load(f)
            data[os.path.basename(subdir)] = len(datum)
    data = {k:v for k, v in sorted(data.items(), reverse=True, key=lambda x: x[1])}
    print('\n'.join(['{}\t{}'.format(k,v) for k, v in data.items()]))
    # print(json.dumps(data, indent=2))
    print('Total event:', sum(data.values()))
    return data


def command(args):
    if args.command == 'count_event':
        count_event(args)
    else:
